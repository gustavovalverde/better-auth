...
export function resolveIssuerWellKnownPath(
	options: Oidc4vciOptions,
): string | null {
...
export function resolvePublicWellKnownPaths(
	options: Oidc4vciOptions,
): string[] {
	...
	if (issuerPath) {
		paths.add(`${WELL_KNOWN_CREDENTIAL_ISSUER_PATH}${issuerPath}`);
	}
	for (const alias of resolveWellKnownAliases(options)) {
		paths.add(alias);
	}
