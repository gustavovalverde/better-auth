...
describe("oidc4vci - well-known aliases", async () => {
	...
	it("serves metadata at issuer-path well-known location", async () => {
		...
		expect(auth.publicHandler).toBeTypeOf("function");
		const res = await auth.publicHandler!(
			new Request(`${baseURL}/.well-known/openid-credential-issuer/api/auth`, {
				method: "GET",
			}),
		);
